...
` + H[0],
  ...
  , xw = ({setActiveStep: n}) => {
    ...
    className:"mb-2",
    children:[
        f.jsx("label",{className:"block text-sm font-medium text-gray-700 mb-2",children:o?.verifyOtp?.[s]||"Enter OTP"}),
        f.jsxs("div",{className:"flex items-center gap-3 rounded-md md:flex-nowrap flex-wrap",children:[
            f.jsx("input",{type:"tel",value:L,onChange:ce=>B(ce.target.value),className:"flex-1 px-3 py-2 text-sm rounded-md border border-gray-300 bg-gray-100 text-gray-700 focus:outline-none",placeholder:o?.lblInputOtp?.[s]||"Enter OTP",autoComplete:"off"}),
            f.jsx("button",{onClick:La,className:"bg-green-600 text-white text-sm font-medium px-4 py-2 rounded-md transition-colors duration-300 hover:bg-green-700 hover:cursor-pointer",children:o?.verify?.[s]||"Verify"})
        ]})
    ]
})
